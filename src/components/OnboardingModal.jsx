...
function StagesPreview() {
  const stages = ['Anchor', 'Apply', 'Reflect', 'Specify', 'Track', 'Adjust', 'Summit'];
  return (
...
const STEPS = [
  ...
  { tag: 'How it works',        title: '7 days. One real output.',     body: "Each sprint has 7 days. The first six build the concept one layer at a time. Day 7 is the Summit, where you produce something concrete that applies to your actual job.", icon: 'none',  hasPreview: 'stages' },
  { tag: 'Do the work', title: 'You write your way through.',  body: "Each day ends with a reflection prompt. No multiple choice, no ratings. Just your honest thinking in your own words. That's what makes it actually land. Here's what it looks like:", icon: 'gate',  hasPreview: 'reflection' },
  ...
  { tag: 'Coach style',          title: 'How should your coach work with you?', body: 'Your Summit Coach adapts to how you think. This setting only affects the coach, not the reading material.',                                                              icon: 'coach', hasPreview: false, isSurveyStyle: true },
      ...
      <div className="onboarding-inner" style={{ background: '#101C2C', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '20px', padding: current.hasPreview ? '36px 40px 32px' : '48px', maxWidth: current.hasPreview ? '520px' : '480px', width: '100%', position: 'relative', transform: exiting ? 'translateY(12px)' : 'translateY(0)', transition: 'transform 0.3s ease', margin: 'auto' }}>
        ...
        {!isLastStep && confirmSkip && (
          <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '10px', padding: '14px 16px', marginBottom: '16px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
            <p style={{ margin: 0, fontSize: '0.82rem', color: 'rgba(238,242,247,0.55)', fontFamily: 'var(--font-sans)', lineHeight: 1.5 }}>
              Are you sure? You won't see this again.
            </p>
            <div style={{ display: 'flex', gap: '8px' }}>
              <button onClick={dismiss}
                style={{ flex: 1, background: 'transparent', border: '1px solid rgba(238,242,247,0.15)', color: 'rgba(238,242,247,0.6)', fontSize: '0.82rem', cursor: 'pointer', padding: '8px', borderRadius: '8px', fontFamily: 'var(--font-sans)', fontWeight: 600, transition: 'all 0.15s' }}
              ...
              <button onClick={() => setConfirmSkip(false)}
                style={{ flex: 1, background: 'rgba(23,184,224,0.08)', border: '1px solid rgba(23,184,224,0.3)', color: '#17B8E0', fontSize: '0.82rem', cursor: 'pointer', padding: '8px', borderRadius: '8px', fontFamily: 'var(--font-sans)', fontWeight: 600, transition: 'all 0.15s' }}
        ...
        {isLastStep && !assignedSprint ? (
          ...
          <button onClick={handleNext}
            disabled={current.isSurveyStyle && !coachStyle}
            style={{ width: '100%', padding: '13px', borderRadius: '10px', border: 'none', background: '#17B8E0', color: '#0D1520', fontSize: '0.93rem', fontWeight: 700, fontFamily: 'var(--font-sans)', cursor: (current.isSurveyStyle && !coachStyle) ? 'not-allowed' : 'pointer', transition: 'opacity 0.15s', opacity: (current.isSurveyStyle && !coachStyle) ? 0.4 : 1 }}
            onMouseEnter={e => { if (!(current.isSurveyStyle && !coachStyle)) e.target.style.opacity = '0.88'; }}
            onMouseLeave={e => { if (!(current.isSurveyStyle && !coachStyle)) e.target.style.opacity = '1'; }}>
