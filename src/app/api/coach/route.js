...
export async function POST(request) {
  try {
    ...
    if (progressRes?.error) {
      return NextResponse.json({ error: `User progress query failed: ${progressRes.error.message}` }, { status: 500 });
    }
    ...
    const recentCompleted = allDays
      .filter(
        (d) =>
          Number(d.day_number) < Number(dayNum) &&
          progressMap[Number(d.day_number)]?.completed
      )
      .slice(-2);
