...
function buildSystemPrompt({ book, currentDay, allDays, userReflection, userMission }) {
  const completedStages = allDays.filter(
    (d) => d.day_number < currentDay.day_number && d.completed
  );
  ...
  const completedSummary =
    completedStages.length > 0
      ? completedStages
          .map(
            (d) =>
              `- Stage ${d.day_number} "${d.title}": ${d.ascent_content?.substring(0, 120)}...`
          )
          .join('\n')
...
export async function POST(request) {
  try {
    ...
    const { bookId, dayNum, userId, userMessage, conversationHistory } =
      await request.json();
    ...
    const [bookRes, currentDayRes, allDaysRes, progressRes] = await Promise.all([
      ...
      supabase
        .from('summit_days')
        .select('day_number, title, ascent_content')
        .eq('book_id', bookId)
        .order('day_number'),
      userId
        ? supabase
    ...
    if (bookRes.error) {
      return NextResponse.json(
        { error: `Books query failed: ${bookRes.error.message}` },
        { status: 500 }
      );
    }
    ...
    if (currentDayRes.error) {
      return NextResponse.json(
        { error: `Current day query failed: ${currentDayRes.error.message}` },
        { status: 500 }
      );
    }
    ...
    if (allDaysRes.error) {
      return NextResponse.json(
        { error: `All days query failed: ${allDaysRes.error.message}` },
        { status: 500 }
      );
    }
    ...
    if (progressRes?.error) {
      return NextResponse.json(
        { error: `User progress query failed: ${progressRes.error.message}` },
        { status: 500 }
      );
    }
    ...
    try {
      ...
      console.error('OpenAI coach error:', openAiError);
      return NextResponse.json(
        { error: `OpenAI failed: ${getErrorMessage(openAiError)}` },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json({
      message: assistantMessage || 'What feels most true for you about today?'
    });
  } catch (error) {
    console.error('Coach API error:', error);
    return NextResponse.json(
      { error: `Coach route crashed: ${getErrorMessage(error)}` },
      { status: 500 }
    );
  }
}